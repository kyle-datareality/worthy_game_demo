...
function applyGlow() {
  ...
  ctx.filter = "blur(5px) brightness(1.5)";
...
function titleScreen() {
  drawDim(0.5);
  if (Math.floor(stateT * 2) % 2 === 0) pixText("START GAME", W / 2, H * 0.5, 24, "#ffd24a");
}
...
function clearScreen() {
  pixText("GEORGE DOWN!", W / 2, H * 0.40, 30, "#ffe24a");
  pixText("+" + (100 * level) + " PTS", W / 2, H * 0.40 + 28, 14, "#41d0ff");
}
function overScreen() {
  drawDim(0.6);
  pixText("GAME OVER", W / 2, H * 0.38, 38, "#ff3b5c");
...
function frame(now) {
  ...
  if (state === STATE.TITLE) {
    if (consume("start") || consume("jump")) newGame();
  ...
  applyGlow();   // bloom on the scene only (UI drawn after, stays sharp)
